...
// ExecuteInteractiveCommand runs a command with stdin/stdout/stderr connected to the OS streams.
// precise for interactive tools like shells or confirmation prompts.
func ExecuteInteractiveCommand(ctx context.Context, name string, arg ...string) error {
	cmd := exec.CommandContext(ctx, name, arg...)
	cmd.Stdin = os.Stdin
