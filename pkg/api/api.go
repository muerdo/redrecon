...
func StartAPI(taskIdentifier string, skipSteps []string, bbotPreset string, proxyManager *utils.ProxyManager, logger *slog.Logger) (string, []string, error) {
...
func stepGatherTargets(state *apiState) error {
	...
	if !utils.FileExistsAndIsNotEmpty(reconTargetsFile) {
		state.logger.Warn("Recon targets file not found or empty, cannot gather API targets.", "path", reconTargetsFile)
		return fmt.Errorf("recon targets file missing: %s", reconTargetsFile)
	}
...
func stepDiscoverSpecs(state *apiState) error {
	...
	proxy := utils.GetProxyForTool(state.proxyManager, "ffuf", state.logger)
	return tools.RunFfuf(state.ctx, state.apiTargetsFile, ffufOutputDir, fuzzWordlist, "", 0, proxy, nil, "", state.logger)
}
...
func stepNucleiAPIScan(state *apiState) error {
	...
	nucleiOutputFile, err := tools.RunNuclei(
		...
		utils.GetProxyForTool(state.proxyManager, "nuclei", state.logger), // proxy (string)
		nil,    // headers
		"",     // cookies
		"", "", // username, password
...
func stepRunBBotAPI(s *apiState, bbotPreset string) error {
	...
	jsonFile, err := tools.RunBBot(
		...
		utils.GetProxyForTool(s.proxyManager, "bbot", s.logger),
		s.logger,
		extraArgs...,
	)
	if err != nil {
