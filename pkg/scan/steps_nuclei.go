...
func stepRunNucleiScan(s *scanState) error {
	...
	nucleiConfig := config.Cfg.Tools.Nuclei
	...
	proxy := utils.GetProxyForTool(s.proxyManager, "nuclei", s.logger)
	...
	nucleiOutputFile, err := tools.RunNuclei(s.ctx, s.urlsFile, strings.Join(templatesToRun, ","), proxy, nil, s.cookies, s.username, s.password, s.logger, extraArgs)
	if err != nil {
