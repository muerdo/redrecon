...
func runMonitoringCycle(ctx context.Context, target string) {
	...
	_, _, _, err := recon.StartRecon(ctx, taskIdentifier, target, nil, nil, bbotPresets, nil, false, true, false, true, false, logger)
	if err != nil {
	...
	if lfrGroup != "" {
		...
		summary, files, err := scan.StartScan(ctx, scanTaskID, newTargetsFile.Name(), nil, []string{"nuclei"}, lfrGroup, "", "", "", false, false, logger)
		if err != nil {
